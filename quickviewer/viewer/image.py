...
class StructLoader:
    ...
    def load_structs(self, structs, names, colors, multi=False):
        ...
        if isinstance(structs, dict):
            ...
            # Load numpy arrays
            array_structs = [name for name in structs if 
                             isinstance(structs[name], np.ndarray)]
    ...
    def add_struct_array(self, array, name, colors):
        """Create StructImage object from a NumPy arrat and add to list."""

        self.loaded = False
        if self.image is None:
            raise RuntimeError("In order to load structs from NumPy array,"
                               " StructLoader class must be created with the "
                               "<image> argument!")

        color = self.find_color_match(colors, name)
        struct = StructImage(array, name=name, color=color, 
